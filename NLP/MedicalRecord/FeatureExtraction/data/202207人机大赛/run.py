...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', type=str, default='231', help='postfix num')
    ...
    result_records = init_with_mrno(file_path=r'labeled_ind_%s.txt' % postfix, with_head=False, sperator='	')

    ############入院记录######################################
    starttime = time.time()
    print('开始处理入院记录....')
    records = ts.load_records('入院记录.txt')
    ts.load_template('../template/入院记录.json')
    ts.set_processor()
    results = ts.process()
    ...
    results = join_others(results, load_dict(r"检验数据.txt"), "检验", fix_date=True)
    results = join_others(results, load_dict(r'超声.txt'), '超声')
    results = join_others(results, load_dict(r'放射.txt'), '放射')
    results = join_others(results, load_dict(r'病理.txt'), '病理', delta=360)
    # results = join_others(results, load_dict(r"data/tmp/yz_%s.txt" % postfix), '医嘱')

    ###############结果写到文件####################################
    ts.write_result('汇总结果_%s.json' % postfix, results)
