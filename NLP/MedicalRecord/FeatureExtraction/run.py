...
from process_inspect_all import load_dict as load_insp_dict
from process_yizhu_all import load_dict as load_yizhu_dict
from process_others import load_dict as load_others_dict
...
if __name__ == "__main__":
    ...
    starttime = time.time()
    print('开始处理入院记录....')
    records = ts.load_records('data/records/选择好入院记录.txt')
    ...
    results = join_others(results, load_insp_dict(), "检验", fix_date=True)
    results = join_others(results, load_others_dict('超声'), '超声')
    results = join_others(results, load_others_dict('放射'), '放射')
    results = join_others(results, load_others_dict('病理'), '病理', delta=360)
    results = join_others(results, load_yizhu_dict(), '医嘱')

    ###############结果写到文件####################################
    ts.write_result('data/汇总结果.json', results)
