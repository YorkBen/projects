...
class ClinicRule(RegexBase):
    def __init__(self, type='腹痛', postfix='4335'):
        ...
        self.features = [
            ...
            {'id':'TYX1', 'name': '头晕', 'src': ['现病史'], 'regex':'头晕',  'default': 2},
            ...
            {'id':'TRFZX1', 'name': '突然发作', 'src': ['现病史'], 'regex':'(突发)|(突然)|(突感)', 'default': 2},
            ...
            {'id':'MFST1', 'name': '墨菲征Murphy征', 'src': ['体格检查', '专科情况（体检）', '查体'], 'regex':'((墨菲征)|(Murphys?征?：?’?))', 'negregex': '((墨菲征)|(Murphys?))' + self.inner + '阴性'},
    ...
    def process(self, json_file, result_file, labeled_file=None, debug=True):
        ...
        self.write_to_excel(results, result_file, debug=debug)
