...
class LabRule(RegexBase):
    ...
    def process_rule_str_regex(self, texts, rule_results, debug=False):
        """
        从自由文本中提取实验室数据特征。
        输入：
            文本数组
    ...
    def process_stack_regex(self, texts, rule_results, debug=False):
        """
        ...
        输出：
            ...
            {rule_id: 0,1,2}
        """
        stack_results = {stack: 2 for stack in self.stack_regex.keys()} if not debug else {stack: (2, '', '', '') for stack in self.stack_regex.keys()}
        for text in texts:
        ...
        # 组套结果赋值给正则结果
        for stack in self.stack_regex.keys():
            rt = stack_results[stack] if not debug else stack_results[stack][0]
            for rule_id in self.stack_rules[stack]:
                rrt = rule_results[rule_id] if not debug else rule_results[rule_id][0]
                if rrt == 2 or rt == 1:
                    rule_results[rule_id] = stack_results[stack]

        return rule_results


    def process_texts(self, texts, debug=False):
        """
        从自由文本中提取实验室数据特征。
        输入：
            文本数组
        输出：
            ...
            {rule_id: 0,1,2}
        """
        rule_results = {k:2 for k in self.rules.keys()} if not debug else {k:(2, '', '', '') for k in self.rules.keys()}

        # rule_str_regex
        rule_results = self.process_rule_str_regex(texts, rule_results, debug)

        # stack regex
        rule_results = self.process_stack_regex(texts, rule_results, debug)
