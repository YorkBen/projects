...
def append_attrs(data, item, label_id, relation_id):
    ...
    for anno in data["annotations"][0]["result"]:
        if anno["type"] == "relation" and anno["from_id"] in mapp and anno["to_id"] in mapp:
            relation_id += 1
            item["relations"].append({
                "id": relation_id,
                "from_id": mapp[anno["from_id"]],
                "to_id": mapp[anno["to_id"]],
                "type": anno["labels"][0] if "labels" in anno and len(anno["labels"]) > 0 else 'null'
            })
        elif anno["type"] == "relation":
            print(data["data"])
            print(anno["labels"][0])
...
def do_convert(args):
    ...
    with open(args.labelstudio_file, "r", encoding="utf-8") as infile:
        ...
        dataset = json.load(infile, strict=False)
        results = convert(dataset, args.task_type)
        # dataset = sorted(dataset, key=lambda x: x['id'])
        # train_len = int(len(dataset) * 0.8)
        # results_train = convert(dataset[:train_len], args.task_type)
        # results_test = convert(dataset[train_len:], args.task_type)

    # with open(args.doccano_file, "w", encoding="utf-8") as outfile:
    # for out_file, data in zip(['doccano_train.jsonl', 'doccano_test.jsonl'], [results_train, results_test]):
