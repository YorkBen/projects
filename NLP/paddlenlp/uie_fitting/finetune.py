        ...
        for batch in train_data_loader:
            ...
            if global_step % args.valid_steps == 0 and rank == 0:
                ...
                if f1 > best_f1:
                    ...
                    tokenizer.save_pretrained(save_dir)
                    logger.enable()
                if record_acc > best_record_acc:
                    logger.info(
                        f"best Record Acc performence has been updated: {best_record_acc:.5f} --> {record_acc:.5f}"
                    )
                    best_record_acc = record_acc
                    save_dir = os.path.join(args.save_dir, "model_best_recordacc")
                    model_to_save = model._layers if isinstance(
                        model, paddle.DataParallel) else model
                    model_to_save.save_pretrained(save_dir)
                    logger.disable()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", choices=["uie-base", "uie-tiny", "uie-medium", "uie-mini", "uie-micro", "uie-nano", "model_best"], default="uie-base", type=str, help="Select the pretrained model for few-shot learning.")
