...
def predict(model, dataloader):
    ...
    preds = np.concatenate(preds)
    labels = np.concatenate(labels)
    # metrics = compute_correlations(labels, preds, None)
    return preds, labels
...
def fit(model,
    ...
    train_loader = DataLoader(
        train_set, batch_size=batch_size, shuffle=False, num_workers=num_workers) ## modified by pengxiang
    ...
    try:
        best_valid_loss, valid_loss = 10000, 10000
        for e in range(max_epochs):
            ...
            if valid_set is not None:
                ...
                criterion = (best_valid_loss > valid_loss)
                ...
                if criterion:
                    ...
                    if path is not None:
                        print('save model: %s' % os.path.join(path, 'model.pt'))
