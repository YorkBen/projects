...
SUMMARY_TITLE_FONT_PATH = "courbd.ttf"
...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  print("tile_sum.tiles", len(tile_sum.tiles))
...
def summary_and_tiles(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True):
  ...
  if save_top_tiles:
    for tile in tile_sum.my_top_tiles():
      tile.save_tile(slide_num)
  return tile_sum
...
def save_display_tile(tile, slide_num, save=True, display=False):
  """
  ...
  if save:
    t = Time()
    img_path = slide.get_tile_image_path(tile, slide_num)
...
def singleprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  if image_num_list is not None:
    ...
    image_num_list, tile_summaries_dict = image_range_to_tiles(0, num_training_slides-1, display, save_summary, save_data,
                                                               save_top_tiles)
...
def multiprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  for num_process in range(1, num_processes + 1):
    ...
    end_index = num_process * images_per_process - 1
...
class TileSummary:
  ...
  def my_top_tiles1(self):
    ...
    top_tiles = sorted_tiles[:NUM_TOP_TILES]
    for t in top_tiles:
      if t.tissue_percentage > 80:
        result_top_tiles.append(t)

    return result_top_tiles

  def my_top_tiles(self):
    """
    Retrieve the top-scoring tiles.
    ...
    #top_tiles = sorted_tiles[:NUM_TOP_TILES]
    for t in self.tiles:
      if t.tissue_percentage > 50:
        top_tiles.append(t)
    if len(top_tiles) > 8000:
      result_top_tiles = random.sample(top_tiles, NUM_TOP_TILES)
    else:
      result_top_tiles = top_tiles

    return result_top_tiles


  def high_quantity_tiles(self):
    high_quantity_tiles = []
    for t in self.tiles:
      if t.tissue_quantity() == TissueQuantity.HIGH:
        high_quantity_tiles.append(t)
    return high_quantity_tiles
...
class Tile:
  ...
  def save_tile(self, slide_num):
    save_display_tile(self, slide_num, save=True, display=False)
