...
FOLD1 = ''
FOLD2 = ''
BASE_DIR = r"/root/data1/zhangkuo"
# BASE_DIR = r"\\192.168.0.60\public\模型算法组\训练数据\zhangkuo\基础AI"
# BASE_DIR = os.path.join(os.sep, "Volumes", "BigData", "TUPAC")
# TRAIN_PREFIX = "TUPAC-TR-"
# SRC_TRAIN_DIR = os.path.join(BASE_DIR, "DX", FOLD1, FOLD2)
SRC_TRAIN_DIR = os.path.join(BASE_DIR, "DX0810", FOLD1, FOLD2)
SRC_TRAIN_EXT = "svs"
DEST_TRAIN_SUFFIX = ""  # Example: "train-"
DEST_TRAIN_EXT = "png"
SCALE_FACTOR = 32
DEST_TRAIN_DIR = os.path.join(BASE_DIR, "DX0810-training", FOLD1, FOLD2)
THUMBNAIL_SIZE = 300
THUMBNAIL_EXT = "jpg"

DEST_TRAIN_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810-training_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)

FILTER_SUFFIX = ""  # Example: "filter-"
FILTER_RESULT_TEXT = "filtered"
FILTER_DIR = os.path.join(BASE_DIR, "DX0810-filter0_" + DEST_TRAIN_EXT, FOLD1, FOLD2)

FILTER_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810-filter_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)

FILTER_PAGINATION_SIZE = 50
FILTER_PAGINATE = True
FILTER_HTML_DIR = BASE_DIR

TILE_SUMMARY_DIR = os.path.join(BASE_DIR, "DX0810-tile_summary_" + DEST_TRAIN_EXT, FOLD1, FOLD2)
TILE_SUMMARY_ON_ORIGINAL_DIR = os.path.join(BASE_DIR, "DX0810-tile_summary_on_original_" + DEST_TRAIN_EXT, FOLD1, FOLD2)

TILE_SUMMARY_SUFFIX = "tile_summary"
TILE_SUMMARY_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810-tile_summary_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)

TILE_SUMMARY_ON_ORIGINAL_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810-tile_summary_on_original_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)

TILE_SUMMARY_PAGINATION_SIZE = 50
TILE_SUMMARY_PAGINATE = True
TILE_SUMMARY_HTML_DIR = BASE_DIR

TILE_DATA_DIR = os.path.join(BASE_DIR, "DX0810-tile_data", FOLD1, FOLD2)

TILE_DATA_SUFFIX = "tile_data"

TOP_TILES_SUFFIX = "top_tile_summary"
TOP_TILES_DIR = os.path.join(BASE_DIR, "DX0810-"+TOP_TILES_SUFFIX + "_" + DEST_TRAIN_EXT, FOLD1, FOLD2)

TOP_TILES_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810"+TOP_TILES_SUFFIX + "_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)

TOP_TILES_ON_ORIGINAL_DIR = os.path.join(BASE_DIR, "DX0810-"+TOP_TILES_SUFFIX + "_on_original_" + THUMBNAIL_EXT, FOLD1, FOLD2)

TOP_TILES_ON_ORIGINAL_THUMBNAIL_DIR = os.path.join(BASE_DIR, "DX0810-"+TOP_TILES_SUFFIX + "_on_original_thumbnail_" + THUMBNAIL_EXT, FOLD1, FOLD2)



TILE_DIR = os.path.join(BASE_DIR, "DX0810-"+ "tiles_" + DEST_TRAIN_EXT, FOLD1, FOLD2)

TILE_SUFFIX = "tile"

STATS_DIR = os.path.join(BASE_DIR, "DX0810-" + "svs_stats", FOLD1, FOLD2)

# print(os.listdir(SRC_TRAIN_DIR))
...
def get_training_slide_path(slide_number):
  ...
  files = os.listdir(SRC_TRAIN_DIR)
...
def get_tile_image_path(tile, slide_num):
  """
  ...
  tile_path = os.path.join(TILE_DIR, files[slide_num][:-4],
                           files[slide_num][:-4] + "#" + TILE_SUFFIX + "-r%d-c%d-x%d-y%d-w%d-h%d" % (
...
def get_training_image_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wildcard_path = os.path.join(DEST_TRAIN_DIR, files[slide_number][:-4] + '#' + "*." + DEST_TRAIN_EXT)
    img_path = glob.glob(wildcard_path)[0]
  else:
    img_path = os.path.join(DEST_TRAIN_DIR, files[slide_number][:-4] + "#" + str(SCALE_FACTOR) + "#" + str(large_w) +
                            "x" + str(large_h) + "#" + str(small_w) + "x" + str(small_h) + "." + DEST_TRAIN_EXT)
  return img_path
...
def get_training_thumbnail_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  """
  files = os.listdir(SRC_TRAIN_DIR)
  img_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, files[slide_number][:-4] + '.png')
  return img_path
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  files = os.listdir(SRC_TRAIN_DIR)
  img_filename = os.path.join(DEST_TRAIN_DIR, files[slide_number][:-4] + 'filter_' + str(filter_number) + "_" +
                              filter_name_info + '.png')
  return img_filename
...
def get_tile_summary_image_filename(slide_number, thumbnail=False):
  ...
  img_filename = files[slide_number][:-4] + "#" + str(SCALE_FACTOR) + "#" + str(large_w) + "x" + str(
...
def get_tile_data_filename(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  data_filename = files[slide_number][:-4] + "#" + str(SCALE_FACTOR) + "#" + str(large_w) + "x" + str(
    large_h) + "#" + str(small_w) + "x" + str(small_h) + "#" + TILE_SUMMARY_SUFFIX + ".csv"
...
def get_filter_image_result(slide_number):
  ...
  # padded_sl_num = str(slide_number).zfill(3)
  training_img_path = get_training_image_path(slide_number)
  # fname = os.path.basename(training_img_path)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  ...
  # img_path = os.path.join(DEST_TRAIN_DIR, files[slide_number][:-4] + "#" + str(
  #   SCALE_FACTOR) + "#" + str(large_w) + "x" + str(large_h) + "#" + str(small_w) + "x" + str(
  #   small_h) + "." + DEST_TRAIN_EXT)

  img_path = os.path.join(FILTER_DIR, files[slide_number][:-4] + "#" + FILTER_RESULT_TEXT + "#" + str(
    SCALE_FACTOR) + "#" + str(large_w) + "x" + str(large_h) + "#" + str(small_w) + "x" + str(
    small_h) + "." + DEST_TRAIN_EXT)
  return img_path
...
def get_filter_thumbnail_result(slide_number):
  ...
  img_path = os.path.join(FILTER_THUMBNAIL_DIR, files[slide_number][:-4] + "-" + FILTER_SUFFIX + "-" + FILTER_RESULT_TEXT + "." + THUMBNAIL_EXT)
  img_path = os.path.join(FILTER_THUMBNAIL_DIR, files[slide_number][:-4] + "#" + FILTER_RESULT_TEXT + "#" + str(
    SCALE_FACTOR) + "#" + str(large_w) + "x" + str(large_h) + "#" + str(small_w) + "x" + str(
    small_h) + "." + THUMBNAIL_EXT)
  return img_path
...
def get_num_training_slides():
  ...
  # print("num_training_slides", SRC_TRAIN_DIR, num_training_slides)
...
def singleprocess_training_slides_to_images():
  ...
  training_slide_range_to_images(0, num_train_images-1)
...
def multiprocess_training_slides_to_images_path(svs_path):
  """
  Convert all WSI training slides to smaller images using multiple processes (one process per core).
  Each process will process a range of slide numbers.
  """
