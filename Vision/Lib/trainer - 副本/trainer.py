...
class Trainer(object):
    ...
    def increase_folders(self, count=20000, split_test=0):
        ...
        increase_fold_imgs(train_dir, train_out, int(file_count * (1 - split_test)), self.train_args)
        if split_test > 0:
            increase_fold_imgs(test_dir, test_out, int(file_count * split_test), self.train_args)

        print('increase folders finished')

    def create_model(self, unfreeze_layers=None):
        ...
        if self.model_type == 'vgg':
            ...
            model.compile(optimizer=Adam(0.001), loss=setting[2], metrics=['accuracy'])
            # sgd = SGD(lr=1e-3, decay=1e-6, momentum=0.9, nesterov=True)
            # model.compile(optimizer=sgd, loss=setting[2], metrics=['accuracy'])
            ...
            # model.compile(optimizer=sgd, loss=setting[2], metrics=['accuracy'])
            # model.compile(optimizer=Adam(0.0001), loss=setting[2], metrics=['accuracy'])
    ...
    def predict_data(self, folds, model=None, childs=None, parent_count=None, preprocess_input=None,
        ...
        for fold in folds:
            _x_test, _y_test, _file_test = self.load_fold_data(fold)
            x_test += _x_test
            y_test += _y_test
    ...
    def export_predict(self, fold, out_fold, min_level=0, max_level=1.0, model=None, target_class_index=None):
        x_test, _y_test, _file_test = load_one_fold_data(fold, size=360)
        if model is None:
