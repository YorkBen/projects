...
from mbsh.core.images import read_img_file
...
class BalloonDataset(utils.Dataset):
    ...
    # for csv
    def load_data_from_csv(self, dataset_dir, csv_name='via_region_data.csv', class_name='region',
                           label=None, exclude_label=None, image_files=None):
        # Add classes. We have only one class to add.
        self.add_class(class_name, 1, class_name)
        img_filename_list = []
        ...
        for i, row in enumerate(filename_col.values):
            img_filename = row[0]
            print(dataset_dir, img_filename, type(dataset_dir), type(img_filename))
            image_path = os.path.join(dataset_dir, img_filename)
            if not os.path.exists(image_path):
                print('file not exists: ', image_path)
                continue
            ...
            for j in range(len(bl_df)):
                if label:
                    try:
                        label_dict = json.loads(bl_df.iloc[j, 6])
                    except:
                        continue
                    if len(label_dict) == 0:
                        continue
                    if label_dict['label'] != label:
                        print('exclulde ', label_dict)
                        continue
                ...
                if exclude_label:
                    try:
                        label_dict = json.loads(bl_df.iloc[j, 6])
                        if label_dict['label'] == exclude_label:
                            print('exclulde ', label_dict)
                            continue
                    except:
                        pass
                ...
                if len(tmp_dict):
                    print(tmp_dict, 'tmp_dict', img_filename)
    ...
    def load_mask_multi(self, image_id, multi=True):
       ...
       Returns:
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            print(p,'pppp', new_dict[p['label']], )
            mask[rr, cc] = new_dict[p['label']] if multi else 1
        # print('aaa mask Counter(data)\n', Counter(mask))
        return mask

    def load_image(self, image_id):
        image = read_img_file(self.image_info[image_id]['path'])
        # image = cv2.imdecode(np.fromfile(self.image_info[image_id]['path'], dtype=np.uint8), -1)
        # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        print(self.image_info[image_id]['path'], image.ndim, image.shape, 'ppppppppppppp')
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for i, p in enumerate(info["polygons"]):
            labeled = True
            # Get indexes of pixels inside the polygon and set them to 1
            if (p['name'] == 'polygon' or p['name'] == 'polyline') and len(p['all_points_y']) != 0:
                print(len(p['all_points_y']), 'len(p[''])')
                rr, cc = skimage.draw.polygon(p['all_points_y'], p['all_points_x'])
                np.minimum(rr, hei-1, out=rr)
                np.minimum(cc, wid - 1, out=cc)
                mask[rr, cc, i] = 1
            elif p['name'] == 'ellipse':
                rr, cc = skimage.draw.ellipse(p['cy'], p['cx'], p['ry'], p['rx'])
                mask[rr, cc, i] = 1
                ...
                rr, cc = skimage.draw.circle(p['cy'], p['cx'], 5)
                mask[rr, cc, i] = 1
            else:
                print('ERROR : ', p['name'])
        ...
        print(mask.shape, info, 'zzzzzzzzzzzz', labeled)
        return mask.astype(np.bool), np.ones([mask.shape[-1]], dtype=np.int32), labeled
    #
